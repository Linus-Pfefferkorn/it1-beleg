...
const katexmüll = document.getElementsByClassName('katex-html')
...
fetch("../JSON/Mathe.json")
    ...
    .then(data => {
        for (let i = 0; i < FileLength; i++) {
            ...
            //Aufgaben
            Question[i][0] = data.Mathe[i].a;
           // console.log("Aufgabe: " + Question[i]);

            for (let j = 1; j <= FileWidth; j++) {

                //Lösungen
                Question[i][j] = data.Mathe[i].l[j - 1];
...
getNewQuestion = () => {
    ...
    katex.render(currentQuestion.question, document.getElementById("question"));

    choices.forEach((choice) => {
        const number = choice.dataset['number'];
        //choice.innerText = currentQuestion['choices'][number - 1];
        katex.render(currentQuestion.choices[number - 1], choice);
    });
